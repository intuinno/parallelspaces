...
QuerySets.prototype = {
    isSelected: function(d, relMin, relMax) {
        if(this.query.indexOf(d) === -1) {
            return false;
        } else if (this.relationMin !== relMin || this.relationMax != relMax) {
...
SelectionStatesSpace.prototype = {
    ...
    isSelected: function (d,relMin, relMax) {
        ...
        for ( i=0;i<this.querySetsList.length;i++) {
            if ( this.querySetsList[i].isSelected(d,relMin, relMax)) {
                return true;
            }
        }
	...
	d3.csv("data/movieSpaceNoNormal.csv", function(movieCSV) {
		...
		for (count = 0; count < movieData.length; count++) {
		...
		movieStar = svgMovieGroup.selectAll("circle")
					...
					.on('click', function(d, i) {
						...
						if (selectionStatesMovie.isSelected(d,PSmin, PSmax) === false) {
		...
		svgUserGroup.selectAll("circle")
			...
			//  console.log((d.numReview*5)*(d.numReview*10));
					...
					.on('click', function(d, i) {
						...
						if (selectionStatesUser.isSelected(d,PSmin, PSmax) === false) {
			                 //Here this star is newly selected 
			                 //So Add to the Selection
