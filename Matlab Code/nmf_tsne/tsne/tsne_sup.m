function ydata = tsne_sup(X, labels, shrink_factor, no_dims, initial_dims, perplexity)
...
%     clear D
    ...
    for i=min(labels):max(labels)
        DD(labels==i,labels==i) = DD(labels==i,labels==i) * shrink_factor;
    end
    
    % Compute joint probabilities
    P = d2p(DD, perplexity, 1e-5);                                           % compute affinities using fixed perplexity
%    clear DD
